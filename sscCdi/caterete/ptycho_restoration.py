...
def restoration_cuda_parallel_cuda(jason):
...
def pi540_restoration_cat(args, savepath = '', preview = False, save = False, first_iteration = True):
    ...
    if preview: # save plots of restaured difpad and mean of all restaured difpads
        difpad_number = 0
        np.save(jason[ 'OutputFolder'] + '/04_difpad_restaured_mean.npy',np.mean(difpads, axis=0))
        np.save(jason[ 'OutputFolder'] + '/04_difpad_restaured.npy',difpads[difpad_number, :, :])
        sscCdi.caterete.misc.plotshow_cmap2(difpads[difpad_number, :, :], title=f'Restaured Diffraction Pattern #{difpad_number}', savepath=jason['OutputFolder'] + '/04_difpad_restaured.png')
        sscCdi.caterete.misc.plotshow_cmap2(np.mean(difpads, axis=0), title=f'Mean Restaured Diffraction Pattern #{difpad_number}', savepath=jason[ 'OutputFolder'] + '/04_difpad_restaured_mean.png')
...
def get_restaurated_difpads_old_format(jason, geometry, path, name,first_iteration,preview):
    ...
    if first_iteration:  # preview only 
        ...
        np.save(jason[ 'OutputFolder'] + '/02_difpad_raw_mean.npy',mean_raw_difpads)
    if preview and first_iteration:
        sscCdi.caterete.misc.plotshow_cmap2(raw_difpads[difpad_number, :, :], title=f'Raw Diffraction Pattern #{difpad_number}', savepath=jason['OutputFolder'] + '/02_difpad_raw.png')
        sscCdi.caterete.misc.plotshow_cmap2(mean_raw_difpads, title=f'Raw Diffraction Patterns mean', savepath=jason['OutputFolder'] + '/02_difpad_raw_mean.png')
    ...
    if preview and first_iteration:
        sscCdi.caterete.misc.plotshow_cmap2(empty, title=f'Empty', savepath=jason['OutputFolder'] + '/01_empty.png')
        sscCdi.caterete.misc.plotshow_cmap2(flat,  title=f'Flat',  savepath=jason['OutputFolder'] + '/01_flat.png')
        sscCdi.caterete.misc.plotshow_cmap2(mask,  title=f'Mask',  savepath=jason['OutputFolder'] + '/01_mask.png')
    ...
    if first_iteration: # difpad used in jupyter to find center position!
        ...
        np.save(jason[ 'OutputFolder'] + '/03_difpad_restaured_flipped.npy',DP0)
        np.save(jason[ 'OutputFolder'] + '/03_difpad_restaured_flipped_masked.npy',DP)
        sscCdi.caterete.misc.plotshow_cmap2(DP, title=f'Restaured Diffraction Pattern #{difpad_number}, pre-binning', savepath=jason['OutputFolder'] + '/03_difpad_restaured_flipped.png')
