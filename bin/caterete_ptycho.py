...
if __name__ == '__main__':
    ...
    restoration_dict_list, restored_data_info_list, filepaths, filenames, folder_names, folder_numbers = sscCdi.caterete.cat_restoration.restoration_ptycho_CAT(input_dict) # restoration of all frames; restored DPs saved at temporary folder    t2 = time.time()
    ...
    object, probe, angles, positions, error = sscCdi.misc.save_volume_from_parts(input_dict)
    ...
    print('\nSaving positions of shape: ',positions.shape)
    ...
    print('\n')
    print(f'Restoration time:     {time_elapsed_restauration:.2f} seconds = {(time_elapsed_restauration) / 60:.2f} minutes ({100*(time_elapsed_restauration)/(t4 - t0):.0f}%)')
    print(f'Ptychography time:     {time_elapsed_ptycho:.2f} seconds = {(time_elapsed_ptycho) / 60:.2f} minutes ({100*(time_elapsed_ptycho)/(t4 - t0):.0f}%)')
