...
if __name__ == '__main__':
    ...
    if len(filenames) > 1 and jason['SerialRestauration'] == False: # 3D batch restauration form (computationally faster, but not memory safe)
        difpads,_ , jason = sscCdi.caterete.ptycho_restoration.restoration_3d(jason) # difpads is a list of size = len(Aquisition_folders)
        t2 = time()
    ...
    if jason['SaveObj']:
        print('Saving Object!')
        sscCdi.caterete.ptycho_processing.save_variable(cropped_sinogram  , os.path.join(jason['ReconsPath'], 'object_' + jason["Acquisition_Folders"][0]))

    if jason['SaveProbe']:
        print('Saving Probe!')
        sscCdi.caterete.ptycho_processing.save_variable(probe, os.path.join(jason['ReconsPath'], 'probe_' + jason["Acquisition_Folders"][0]) )
